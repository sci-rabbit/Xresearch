...
class AxiomApi:
    ...
    def __init__(
        ...
        self.client = AxiomRequest(session=session)
    ...
    async def fetch_holder_data(
        ...
        try:
            axiom_data = await self.client.fetch(
                holder_data_url,
                refresh_url,
                headers=self.HEADERS,
                cookies=self.COOKIES,
    ...
    async def get_info_about_token(self) -> dict[str, Any]:
        ...
        """
        token_info_task = asyncio.create_task(self.fetch_token_info())
        top_holder_task = asyncio.create_task(self.fetch_holder_data())
        pair_info_task = asyncio.create_task(self.fetch_pair_info())
